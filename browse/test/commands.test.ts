...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/rkstack-cli-');
    const execPath = path.join(root, '.claude/skills/rkstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/rkstack/browse/src/server.ts');
