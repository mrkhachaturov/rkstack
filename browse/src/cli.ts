/**
 * rkstack-browse CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .rkstack/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and rkstack
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`rkstack-browse — Fast headless browser for AI coding agents
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      try {
        ...
        const agentQueue = path.join(process.env.HOME || '/tmp', '.rkstack', 'sidebar-agent-queue.jsonl');
