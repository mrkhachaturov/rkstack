/**
 * rkstack-browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.rkstack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.rkstack/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['rkstack-browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.rkstack', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'plugins', 'rkstack', 'browse', 'dist', 'rkstack-browse'),
    path.join(process.env.HOME || '', '.claude', 'plugins', 'rkstack', 'browse', 'dist', 'rkstack-browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/plugins\/rkstack\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.rkstack', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.rkstack', 'sidebar-agent-queue.jsonl');
  const queueDir = path.dirname(agentQueue);
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.rkstack', 'chromium-profile');
