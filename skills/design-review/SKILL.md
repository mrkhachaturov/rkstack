...
**If test framework detected:** Read 2-3 existing test files to learn conventions. Store for use in Phase 8e.5. **Skip bootstrap.**
**If BOOTSTRAP_DECLINED:** Skip bootstrap.
**If no test framework and no opt-out:** Follow the same bootstrap procedure as /qa (research, select, install, verify, commit).
...
18. **DESIGN.md is law.** If it exists, every finding must reference whether it aligns or deviates from the stated design system.
19. **DESIGN.md export.** You MAY write a DESIGN.md file if the user accepts the offer from Phase 2.
