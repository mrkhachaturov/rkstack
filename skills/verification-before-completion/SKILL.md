...
1. **Quick QA.** Invoke the `/qa-only` skill in quick mode (critical and high severity only). This produces a health score and screenshots. If health score is below 80%, flag it as a concern in the completion report.

2. **Responsive check.** Run `$RKSTACK_BROWSE responsive` on key pages. Verify no layout breakage at mobile (375px), tablet (768px), and desktop (1280px).

3. **Console audit.** Run `$RKSTACK_BROWSE console --errors` across key pages. Any JavaScript errors are flagged.
