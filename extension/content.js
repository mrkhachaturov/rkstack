...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'rkstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="rkstack-pill-dot"></span> rkstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'rkstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'rkstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'rkstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'rkstack-ref-panel-header';
  header.textContent = `rkstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'rkstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'rkstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'rkstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'rkstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'rkstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'rkstack-ref-panel-more';
