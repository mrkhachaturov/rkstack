...
const CAREFUL_SCRIPT = path.join(ROOT, 'skills', 'careful', 'bin', 'check-careful.sh');
const FREEZE_SCRIPT = path.join(ROOT, 'skills', 'freeze', 'bin', 'check-freeze.sh');
...
function withFreezeDir(freezePath: string, fn: (stateDir: string) => void) {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rkstack-freeze-test-'));
