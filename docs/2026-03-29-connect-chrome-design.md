...
**`findExtensionPath()`** -- checks 3 locations in order using `os.homedir()` + `path.join()` (cross-platform, no tilde expansion): repo root `extension/`, plugin install `<homedir>/.claude/plugins/rkstack/extension/`, home `<homedir>/.rkstack/extension/`. Returns first that exists, or null. Works on macOS, Linux, and Windows.
...
**`useragent` in headed mode** -- `recreateContext()` throws in headed mode because persistent context cannot be recreated without losing the extension. The `useragent` command must detect headed mode and reject with: "Cannot change user agent in headed mode. Run `disconnect` first, change the user agent, then `connect` again."

### Disconnect state machine

Who sets `intentionalDisconnect` and what happens:

| Trigger | Who sets it | Exit code | CLI behavior |
|---------|------------|-----------|-------------|
| `disconnect` command | meta-command handler, before shutdown | 0 (clean) | Next command starts headless |
| `close()` during handoff | `handoff()`, before closing old browser | N/A (no exit) | New headed browser takes over |
| User closes Chrome window | Nobody (not intentional) | 2 | CLI prints "Browser closed. Run `connect` to reconnect." Does NOT auto-restart. |
| Chromium crashes | Nobody | 1 | CLI prints crash message. Auto-restart in headless only if state was `launched`, not `headed`. |

The `sendCommand()` retry logic in cli.ts must check `state.mode` before auto-restarting. If mode was `headed`, do not silently restart in headless -- print an error and exit.

### Handoff preservation matrix

What survives the headless-to-headed transition:

| State | Preserved? | Reason |
|-------|-----------|--------|
| Cookies | Yes | Restored via `restoreState()` |
| localStorage / sessionStorage | Yes | Restored via `restoreState()` |
| Open URLs / active tab | Yes | Restored via `restoreState()` |
| Extra HTTP headers | Yes | Applied to new context via `setExtraHTTPHeaders()` |
| Custom user agent | No | Persistent context uses its own UA |
| Ref map | No | New pages, old locators are stale |
| Snapshot baseline | No | Reset -- new page state |
| Frame context | No | Reset -- frames are per-page |
| Dialog auto-accept | No | Resets to `false` (user controls dialogs in headed mode) |
| Consecutive failure count | No | Reset to 0 |
...
Already returns `mode`. Add: `chatEnabled`, `agent: { status, runningFor, currentMessage, queueLength }`, `session: { id, name }`, and `token` (the Bearer auth token).

### Auth bootstrap for extension and sidebar agent

The auth token is a random UUID generated by the server at startup and written to the state file. Three consumers need it:

- **CLI** -- reads token from state file (existing behavior, no change)
- **Chrome extension** -- reads token from the `/health` endpoint response. `/health` is unauthenticated (localhost-only) and includes `token` in the JSON response. The extension stores it and uses it for all subsequent `/sidebar-*` and `/command` requests.
- **Sidebar agent** -- reads token from the state file at startup (passed via `BROWSE_STATE_FILE` env). Refreshes by polling `/health` if the token changes (server restart).

This matches gstack's proven pattern. The token is only accessible from localhost, so exposure risk is limited to local processes.
...
3. Clean Chromium profile locks (via `path.join(os.homedir(), '.rkstack', 'chromium-profile', 'SingletonLock')` etc. -- cross-platform)
...
Add `mode?: 'launched' | 'headed'` field. Older state files without `mode` are treated as `'launched'` (backward compatible -- missing field defaults to headless).
